...
import { ASNItem } from "@/contexts/data-context";
...
export function ASNAddForm({
  ...
  const validateForm = () => {
    ...
    if (!formData.ip.trim()) {
      ...
      newErrors.ip = "请输入有效的 IPv4 或 IPv6 地址";
    }

    if (!formData.asn.trim()) {
      newErrors.asn = "ASN 不能为空";
    } else if (!validateASN(formData.asn.trim())) {
      newErrors.asn = "ASN 格式应为 AS + 数字，例如：AS15169";
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="asn-ip">IP 地址 *</Label>
        <Input
          id="asn-ip"
          value={formData.ip}
      ...
      <div className="space-y-2">
        <Label htmlFor="asn-number">ASN 编号 *</Label>
        <Input
          id="asn-number"
          value={formData.asn}
        ...
        />
        {errors.asn && <p className="text-sm text-destructive">{errors.asn}</p>}
        <p className="text-xs text-muted-foreground">
          自治系统编号，格式为 AS + 数字
        </p>
      </div>
      ...
      <DialogFooter>
        ...
        <Button type="submit">添加 ASN 映射</Button>
      </DialogFooter>
    </form>
  );
}
...
export function ASNEditForm({
  ...
  const [formData, setFormData] = useState<ASNFormData>({
    ip: asnData.ip,
    asn: asnData.asn,
    enabled: asnData.enabled, // 使用现有的 enabled 状态
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="edit-asn-ip">IP 地址 *</Label>
        <Input
          id="edit-asn-ip"
          value={formData.ip}
          onChange={(e) => setFormData({ ...formData, ip: e.target.value })}
      ...
      <div className="space-y-2">
        <Label htmlFor="edit-asn-number">ASN 编号 *</Label>
        <Input
          id="edit-asn-number"
          value={formData.asn}
          onChange={(e) => handleASNChange(e.target.value)}
...
export function ASNBulkImportForm({
  ...
  const validateAndParseBulkData = (data: string): ASNFormData[] => {
    ...
    lines.forEach((line, index) => {
      ...
      if (!ipv4Regex.test(ip) && !ipv6Regex.test(ip)) {
        newErrors.push(`第${index + 1}行：IP 地址格式错误`);
        return;
      }
      ...
      if (!/^AS\d+$/.test(normalizedASN)) {
        newErrors.push(`第${index + 1}行：ASN 格式错误`);
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="bulk-asn-data">批量 ASN 数据 *</Label>
        <Textarea
          id="bulk-asn-data"
          value={bulkData}
          onChange={(e) => setBulkData(e.target.value)}
          placeholder="每行一个 IP 和 ASN 映射，用空格或制表符分隔&#10;例如:&#10;8.8.8.8 AS15169&#10;1.1.1.1 AS13335&#10;114.114.114.114 AS4134"
          rows={8}
          className={errors.length > 0 ? "border-destructive" : ""}
        />
        <p className="text-xs text-muted-foreground">
          每行格式：IP 地址 ASN 编号（用空格分隔）
        </p>
      </div>
